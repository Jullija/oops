...
def insert_user_groups(hasura_url, headers, users, roles, groups, students_in_group_count, random):
    ...
    for group_id, student_count in zip(groups, students_in_group_count):
        for _ in range(student_count):
            if student_index < len(student_ids):
                ...
                print(f"  Assigning student {student_id} to group {group_id}")
                mutation = """
                mutation MyMutation($userId: bigint!, $groupId: bigint!) {
                    insertUserGroups(objects: {userId: $userId, groupId: $groupId}) {
                        affectedRows
                    }
                }
                """
                ...
                data = response.json()
                if "errors" in data:
                    print(f"    Error inserting user_group for student {student_id} in group {group_id}: {data['errors']}")
    ...
    print("Assigning teachers and the coordinator to groups...")
    ...
    for group_id in groups:
        ...
        print(f"  Assigning teacher/coordinator {assigned_teacher} to group {group_id}")
        mutation = """
        mutation MyMutation($userId: bigint!, $groupId: bigint!) {
            insertUserGroups(objects: {userId: $userId, groupId: $groupId}) {
                affectedRows
        ...
        data = response.json()
        if "errors" in data:
            print(f"    Error inserting user_group for teacher/coordinator {assigned_teacher} in group {group_id}: {data['errors']}")
